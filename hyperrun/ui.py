...
# %% auto #0
__all__ = ['icons', 'chats', 'app', 'rt', 'home', 'chat_content', 'chat_panel', 'settings_panel', 'save_settings',
           'model_options', 'close_chat', 'clear_messages', 'user_bubble', 'ai_bubble_stream', 'ask', 'stream_response']
...
# %% ../nbs/01_ui.ipynb #f38dc43c
@rt("/", methods=["GET"])
def home():
    return Div(
        icons,  # SVG sprite defs
        ...
        Div(cls="modal", role="dialog")(
            chat_content(),
            Label("close", fr="chat-modal", cls="modal-backdrop")
        ),
    )
...
def chat_content():
    return Div(
        Div(
            cls="flex items-center justify-between px-6 py-5 border-b border-base-200"
        )(
        ...
        Div(
            id="messages",
            cls="overflow-y-auto overflow-x-hidden h-[52vh] px-6 py-5 space-y-4 bg-base-100",
        ),
        ...
        cls="modal-box w-11/12 max-w-4xl p-0 bg-base-100 border border-base-300 rounded-3xl shadow-[0_25px_80px_-20px_rgba(0,0,0,0.45)]",
    )
...
def ask(query: str, sess):
    ...
    return Div(user_bubble(query), ai_bubble_stream(query, msg_id, model_name, total_cost)), new_inp
...
async def stream_response(query: str, msg_id: str, sess):
    ...
    async def generate():
        ...
        if final_response is not None:
            try:
                resp_cost = float(completion_cost(completion_response=final_response) or 0.0)
            except Exception:
        ...
        yield sse_message(Span(f"${total_cost:.6f}", id=f"cost-{msg_id}", hx_swap_oob="true"))
